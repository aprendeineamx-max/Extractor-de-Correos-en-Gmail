 ...
 *
 * Columnas "Correo":
 * From | To | Cc | Bcc | Subject | Snippet | BodyURLs | Date | Direction | Folder |
 * Unread | LabelIds | PlainText | HtmlBody | URLs | Attachments | ThreadId | MessageId
 ************************************************************/
...
function authorizeOnce() {
  // FUENTE: el perfil devuelve email e historyId. (Gmail API)
  ...
  do {
    ...
    if (pending.length) {
      ...
      pending.forEach(id => {
        ...
        // Cuerpos completos (HTML y Texto) + fallback a GmailApp
        ...
        // Snippet = CUERPO COMPLETO solicitado
        const snippet = plainBody;

        // URLs desde HTML y texto (con redirecciones desenrolladas)
...
function fullRescanAll() {
  ...
  do {
    ...
    ids.forEach(id => {
      ...
      const htmlBody  = truncate_(bodies.html || '', MAX_CELL);
      const plainBody = truncate_(bodies.plain || '', MAX_CELL);
      const snippet   = plainBody; // Cuerpo completo solicitado
 ...
 **********************/
// Devuelve {html, plain} buscando multipartes y con fallback a GmailApp
function getBodies_(messageId, payload) {
  // 1) Preferimos el contenido de la API avanzada en formato FULL:
  // - El HTML y texto suelen venir en MIME parts y, si no hay data, como attachmentId base64url. (docs)
  // 2) Si falta algo, usamos GmailApp como respaldo (getBody/getPlainBody). (docs)
  ...
  if (!html || !text) {
    try {
      ...
      if (!html) html = msg.getBody();        // HTML completo (con emojis)
      if (!text) text = msg.getPlainBody();   // Texto plano
    } catch (_) {}
  }

  // Si tenemos HTML pero el texto plano es muy corto, derivarlo del HTML conservando saltos/emojis
...
// HTML → texto legible (saltos, listas, entidades, conserva emojis)
...
function decodeB64Url_(data) {
  ...
  return Utilities
    ...
    .getDataAsString('UTF-8'); // fuerza UTF-8 para emojis y acentos
}
...
// Normaliza URL de redirecciones comunes (google.com/url?q=..., .../url?url=...)
function normalizeRedirect_(url) {
  try {
    const u = new URL(url);
    // enlaces de Google redirect
...
// Extrae HREFs del HTML + URLs visibles en texto/HTML
function extractUrlsFromHtmlAndText_(html, plain) {
  ...
  const push = (u) => {
    ...
    if (url.startsWith('//')) url = 'https:' + url; // scheme-less → https
    // Solo http(s)
  ...
  // href="..." y href='...' + data-saferedirecturl
  ...
  // URLs visibles en HTML/texto (incluye las que no están ligadas)
  const both = `${h}\n${p}`;
...
// Cabeceras → objeto (claves canónicas)
function headersToObject_(arr) {
  ...
  (arr || []).forEach(h => {
    ...
    if (canon[lc]) o[canon[lc]] = val; // canónica
  });
  return o;
}
...
function ensureHeaderAndFilter_() {
  ...
  const headers = [
    'From','To','Cc','Bcc','Subject','Snippet','BodyURLs','Date',
    'Direction','Folder','Unread','LabelIds',
    'PlainText','HtmlBody','URLs','Attachments','ThreadId','MessageId'
  ];
